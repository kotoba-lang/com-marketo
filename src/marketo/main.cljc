(ns marketo.main
  "Kotodama WASM entrypoint for the Marketo clean-room actor (L4) — Clojure port.
  ...
  facts (`marketo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "marketo")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"marketo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "marketo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :marketo-compat/ready)
